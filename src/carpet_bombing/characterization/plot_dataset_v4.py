...
TARGET_DESTINATIONS = [f"10.0.0.{i}" for i in range(1, 81)]
COLORS = {
    "Actives": "#1f77b4",
    "Inactives": "#ff7f0e",
    "Normal": "#2ca02c",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate V4 characterization graphs.")
    parser.add_argument("--input", required=True)
    parser.add_argument("--output-dir", default="docs/figures/v4")
...
def plot_attack_source_destination_map(dataset, output_dir):
    ...
    plt.xticks(range(len(TARGET_DESTINATIONS)), TARGET_DESTINATIONS, rotation=90, fontsize=5)
    plt.yticks(range(len(source_order)), source_order)
    for boundary in (19.5, 39.5, 59.5):
        plt.axvline(boundary, color="#333333", linestyle="--", linewidth=1)
    save_current_figure(output_dir, "04_attack_source_destination_map.png")
