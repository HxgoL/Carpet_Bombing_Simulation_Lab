...
CAPTURE_FILTER = "net 10.10.0.0/24 or net 10.20.0.0/24 or net 172.16.0.0/16"


def parse_args():
    parser = argparse.ArgumentParser(description="Run a V5 capture scenario.")
    parser.add_argument(
        "--scenario",
...
def cleanup_mininet():
    ...
    subprocess.run(
        ["pkill", "-f", "fragmented_carpet_bombing.py"],
        cwd=REPO_ROOT,
...
def capture_scenario(scenario, labels, args):
    ...
    warmup = 0 if scenario == "normal" else args.warmup if args.warmup is not None else labels["warmup_seconds"]
