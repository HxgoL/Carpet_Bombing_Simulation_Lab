...
Cette version reprend la V5 stable et ajoute quelques apports non-Docker de la V5.2.

La V5.3 ne contient pas encore Docker. Elle complète la V5 avec :
...
- des récepteurs TCP/UDP simples sur les victimes actives.

## Objectif

La V5.3 sert à produire des captures plus riches pour la caractérisation, la détection et la mitigation.
...
```bash
a1 python3 src/carpet_bombing/simulations/simulation_v5_3/traffic_generator/advanced_carpet_bombing_attack.py \
...
La V5.3 ne dépend pas de Docker ou Containernet. Les services applicatifs réels pourront être ajoutés plus tard avec le travail Docker/FastAPI.
